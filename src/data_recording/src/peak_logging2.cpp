...
void angleCallback(const sensor_msgs::JointState::ConstPtr& msg) 
{
	angle = msg->position[2];	 
	if(angle>0)
	{
		config = "right";
...
void latitudeCallback(const sensor_msgs::NavSatFix::ConstPtr& msg)  //change this to GPS data type
{
	gps_latitude = msg->latitude;	  //change this to GPS data!
...
void startCallback(const std_msgs::Int16::ConstPtr& msg)
{
	...
	ROS_INFO_STREAM("TO WRITE TO LOG FILE, FORCE = " << max);
	prior = 0;
	flag1 = 0;
	flag2 = 0;
	flag3 = 0;
	...
	//sub = nh.subscribe("fix", 1000, latitudeCallback);  //change subscription topic to GPS!!
	//while(flag3==0)
	//{
	//	ros::spinOnce();
	//}
