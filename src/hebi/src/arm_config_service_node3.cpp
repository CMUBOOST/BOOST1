...
public:
    ...
    void callback(const sensor_msgs::JointState::ConstPtr& msg)
    {
        ...
        float joint2_rightconfig = -0.5;
        float joint1_stowconfig = 0.5;
        float joint2_stowconfig = -2.8;
        ...
        if (direction == "left")
        {
            leftFunction(msg, joint1_leftconfig, joint2_leftconfig, joint2_stowconfig);
        }
...
void leftFunction(const sensor_msgs::JointState::ConstPtr& msg, float joint1_leftconfig, float joint2_leftconfig, float joint2_stowconfig)
{
    sensor_msgs::JointState jointActuate;
    jointActuate.header.stamp = ros::Time::now();
    jointActuate.header.frame_id = "";
    ...
    else if ((msg->position[2] >= joint1_leftconfig) && (stowToLeft == 0))
    {
      ROS_INFO("IF 3");
    ...
    else if ((msg->position[2] < joint1_leftconfig) && (msg->position[0] < 0.5))
    {
      ROS_INFO("IF NEW");
    ...
    else if (msg->position[2] < joint1_leftconfig)
    {
      ROS_INFO("IF NEW2");
      jointActuate.velocity.push_back(0.0);
      jointActuate.velocity.push_back(0.0);
    ...
    /*
    else if ((msg->position[2] > joint2_stowconfig) && (stowToLeft == 0) && (msg->position[0] <= 0.5))  //This was added to catch a far left configuration. Geez this is getting ugly!!
    {
      ROS_INFO("IF NEW!!");
      jointActuate.velocity.push_back(5);
      jointActuate.velocity.push_back(0.0);
    ...
    else if ((msg->position[2] < joint1_leftconfig) && (stowToLeft == 0))  //This was added to catch a far left configuration. Geez this is getting ugly!!
    {
      ROS_INFO("IF NEW2!!");
      jointActuate.velocity.push_back(0.0);
      jointActuate.velocity.push_back(0.0);
    ...
    else if ((msg->position[2] <= joint2_stowconfig) && (msg->position[0] < 0.5))
    {
      ROS_INFO("IF 4");
      jointActuate.velocity.push_back(5);
      jointActuate.velocity.push_back(0.0);
    ...
    else if ((msg->position[2] <= joint1_leftconfig) && (stowToLeft == 1))
    {
      ROS_INFO("IF 5");
      jointActuate.velocity.push_back(0.0);
      jointActuate.velocity.push_back(0.0);
