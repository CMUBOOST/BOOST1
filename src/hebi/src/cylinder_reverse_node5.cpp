...
public:
	...
	void callback(const sensor_msgs::JointState::ConstPtr& msg)
	{
        ...
        */
          ...
          if ((direction == "left") && (msg->position[4] < j1_Lconfig) && (count<400))
		  {
            ROS_DEBUG_STREAM("Count is: " << count);

            float gain= float(count)/500;
            float mPError1 = j1_Lconfig-msg->position[4];
            float command1 = msg->position[4]+gain*mPError1;
            ...
            float camera_x = hyp_camera*cos(command1 + msg->position[6]) + joint2_x;
            float camera_y = hyp_camera*sin(command1 + msg->position[6]) + joint2_y;
            ...
            jointActuate.position.push_back(msg->position[6] - line_diff);
            pub_.publish(jointActuate);
            ROS_DEBUG_STREAM("In the left IF statement: "  << (msg->position[6] - line_diff));
          ...
          else if ((direction == "left") && (count<480))
		  {
            ...
            float mPError2 = abs(j2_Lconfig-msg->position[6]);
            float command2 = msg->position[6]+gain*mPError2;
            jointActuate.position.push_back(NAN);
            jointActuate.position.push_back(NAN);
            jointActuate.position.push_back(NAN);
            jointActuate.position.push_back(NAN);                                    
            ...
            pub_.publish(jointActuate);
            ROS_DEBUG_STREAM("In the left ELSEIF statement" << j2_Lconfig << ", " << msg->position[6] << ", " << mPError2 << ", " << command2);
		  }  
          ...
          else if ((direction == "right") && (msg->position[4] > j1_Rconfig) && (count<400))
		  {
            ROS_DEBUG_STREAM("Count is: " << count);

            float gain= float(count)/500;
            float mPError1 = j1_Rconfig-msg->position[4];
            float command1 = msg->position[4]+gain*mPError1;
		  ...
		  {
              ROS_DEBUG_STREAM("IF2, J1 POSITION: " << msg->position[4]);
              float gain = float(count-400)/80.0;
              float mPError2 = abs(j2_Rconfig-msg->position[6]);
              float command2 = msg->position[6]+gain*mPError2;
              jointActuate.position.push_back(NAN);
              jointActuate.position.push_back(NAN);
              jointActuate.position.push_back(NAN);
