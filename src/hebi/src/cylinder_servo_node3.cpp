...
void myCallback(const boost::shared_ptr<const JointState>& jointStatePtr)
{
	...
	else if (direction == "right")
	{
		...
		mIError1 += theta1_R-jointStatePtr->position[0];

		mPError2 = theta2_R-jointStatePtr->position[2];
		mIError2 += theta2_R-jointStatePtr->position[2];
         ...
         hebi::ArmServo::Response &res)
{
  ...
  message_filters::Subscriber<PointStamped> boardPointSub(nh, "/kdc/board_point", buffer);
  ...
  ros::NodeHandle n;
  ros::Subscriber sub = n.subscribe("/joint_states", 1, myCallback);
