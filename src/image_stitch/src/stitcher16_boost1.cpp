...
private:
	...
	pcl::PointCloud<pcl::PointXYZRGB>::Ptpcl::PointXYZRGB>::Ptr cloud_full, Eigen::Matrix4f GlobalTransform_odom);
	std::vector<std::vector<float> > hotspotfilterFunction(pcl::PointCloud<pcl::PointNormal>::Ptr cloud, int n);
	std::vector<std::vector<float> > sphereLogic2(std::vector<std::vector<std::vector<float> > > sphere_locs_3d);
	//bool alphaService(image_stitch::AlphaCentroid::Request  &req, image_stitch::AlphaCentroid::Response &res);
// ---	
private:
	...
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_full;r cloud_filtered_old;
	...
	const static int rMin = 68; //99; //15; //31;
	const static int gMax = 255;
	const static int gMin = 78; //115; //15; //22;
	const static int bMax = 255;
	const static int bMin = 44; //56; //15; //22;
	...
	const static float z_threshold = 0.02; //0.02 //this determines the angle of the stalk
	const static float x_threshold = 0.02; //0.02 //this determines the angle of the stalk
	    ...
	    for(int q=0; q<hotspots.size(); q++){
	    	if(hotspots[q][0]!=0){
				...
				if(sqrt(pow(centroid_x,2)+pow(centroid_y,2))<arm_extended && height_middle<(middle_start-0.2)){
			        ...
			        output.position.push_back(NAN);
			        output.position.push_back(NAN);
			        output.position.push_back(-50*height_middle);
			        output.position.push_back(NAN);
			        zPub.publish(output);
			        sleep(3);
