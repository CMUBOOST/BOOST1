...
private:
	...
	const static int collision_thresh = 100;
};
// END .h
...
//-------------------Apply filtering based on normals and x-density-----------------------
	...
	hotspots_int_save.resize(num_centroids_layer, std::vector<float>(3, initial_value)); //this was 10!!	
    ...
    for(int n=0; n<num_slices; n++){
	    ...
	    for(int q=0; q<hotspots.size(); q++){
	    	if(hotspots[q][0]!=0){		
		    	...
		    	//ROS_DEBUG_STREAM("Histogram x_loc: " << sphere_locs_3d[n][q][0] << ", Histogram y_loc: " << sphere_locs_3d[n][q][2]);	
				...
				if(sqrt(pow(centroid_x,2)+pow(centroid_y,2))<arm_extended && height_middle<(middle_start-0.2) && greenavg > greenthresh && pointIdxRadiusSearch.size() > 20){
				    ...
				    float slope = (centroid_y-0.193)/(centroid_x-0.229);
		    		...
		    		ROS_WARN_STREAM("Elbow start x: " << 0.229 << ", Elbow start y: " << 0.193);
		    		...
		    		//std::cout << matrix_save.size() << std::endl;
		    		float new_x = 0;
		    		float new_y = 0;
		    		...
		    		for(int i = hotspots_int_save[q][0]; i>-1; i--){
		    			if(new_y>=0){
			    			new_x = i;
			    			new_y = orig_y - (1/slope)*counter;
			    			std::cout << "new_x: " << new_x << ", new_y: " << int(new_y+0.5) << ", value: " << matrix_save[int(new_y+0.5)][i] << std::endl;
			    			counter++;
			    			if(counter>2 && matrix_save[int(new_y+0.5)][i]>collision_thresh && matrix_save[int(new_y+1.5)][i]>collision_thresh && matrix_save[int(new_y-0.5)][i]>collision_thresh && matrix_save[int(new_y+2.5)][i]>collision_thresh && matrix_save[int(new_y-1.5)][i]>collision_thresh){
			    				flag = 1;
			    			}
			    		}
		    		}
					...
					hebi::Stage srv;
					srv.request.stage_req = -24.1*height_middle;
