	...
	*/
	...
	for(int m=0; m<cloud_vox_dist_color_new->size(); m++){
		...
		*/
		...
		if(cloudWithNormals->points[m].normal[1] < 0.2 && cloudWithNormals->points[m].normal[1] > -0.1){
	    ...
	    for(int q=0; q<hotspots.size(); q++){
	    	if(hotspots[q][0]!=0){		
				...
				if(sqrt(pow(centroid_x,2)+pow(centroid_y,2))<arm_extended && height_middle<(middle_start-0.2) && greenavg > greenthresh && pointIdxRadiusSearch.size() > 20){
					...
					searchPoint2.x = x_avg;
					searchPoint2.y = y_avg;
					searchPoint2.z = z_avg;
					pointIdxRadiusSearch.clear();
					pointRadiusSquaredDistance.clear();

					std::cout << "Above searchpoint" << std::endl;

					//if ( kdtree.radiusSearch (searchPoint2, radius, pointIdxRadiusSearch, pointRadiusSquaredDistance) > 0 ){
   					//	ROS_INFO_STREAM("Number of inliers: " << pointIdxRadiusSearch.size ());
					//}

					//pcl::PointCloud<pcl::PrincipalCurvatures>::Ptr curve_sum(new pcl::PointCloud<pcl::PrincipalCurvatures> ());
					float curve_x = 0;
					float curve_y = 0;
					float curve_z = 0;
					float curve_1 = 0;
					...
					kdtree.radiusSearch (searchPoint2, radius, pointIdxRadiusSearch, pointRadiusSquaredDistance);
					for(int m=0; m<pointIdxRadiusSearch.size(); m++){
						//std::cout << principalCurvatures->points[pointIdxRadiusSearch[m]] << std::endl;
						//std::cout << principalCurvatures->points[pointIdxRadiusSearch[m]].principal_curvature[0] << ", " << principalCurvatures->points[pointIdxRadiusSearch[m]].pc2 << std::endl;
						curve_x+=principalCurvatures->points[pointIdxRadiusSearch[m]].principal_curvature[0];
						curve_y+=principalCurvatures->points[pointIdxRadiusSearch[m]].principal_curvature[1];
						curve_z+=principalCurvatures->points[pointIdxRadiusSearch[m]].principal_curvature[2];
						curve_1+=principalCurvatures->points[pointIdxRadiusSearch[m]].pc1;
						curve_2+=principalCurvatures->points[pointIdxRadiusSearch[m]].pc2;
					}

					curve_x = curve_x/pointIdxRadiusSearch.size();
					curve_y = curve_y/pointIdxRadiusSearch.size();
					curve_z = curve_z/pointIdxRadiusSearch.size();
					curve_1 = curve_1/pointIdxRadiusSearch.size();
					curve_2 = curve_2/pointIdxRadiusSearch.size();

					//std::cout << "Curve x: " << curve_x << ", Curve y: " << curve_y << ", Curve z: " << curve_z << ", Curve 1: " << curve_1 << ", Curve 2: " << curve_2 << std::endl;																				
					std::cout << curve_x << " " << curve_y << " " << curve_z << " " << curve_1 << " " << curve_2 << std::endl;
					*/
