...
public:
	...
	std::vector<std::vector<float> > hotspotfilterFunction(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int n);
...
private:
	...
	//------------Cloud Slicing------------
	const static int rMin_slice = 15; //99; 
	const static int gMin_slice = 15; //115;
	const static int bMin_slice = 15; //56;
...
void Stitch::pairAlign (const pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_src, const pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_tgt, const pcl::PointCloud<pcl::PointXYZRGB>::Ptr output, Eigen::Matrix4f &final_transform, bool downsample)
{
  	...
  	*/
  	...
  	while (reg.getMaxCorrespondenceDistance() > pairAlign_correspondenceDistance_cutoff)
  	{
    	...
    	//std::cout << "max correspondence: " << reg.getMaxCorrespondenceDistance() << std::endl;
...
std::vector<std::vector<float> > Stitch::hotspotfilterFunction(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int n)
{
...
void Stitch::slice(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_full, Eigen::Matrix4f GlobalTransform_odom){
    ...
    // apply filter
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_full_dist_color (new pcl::PointCloud<pcl::PointXYZRGB>);	
    condrem.filter (*cloud_full_dist_color);
    std::vector<int> indices;
	pcl::removeNaNFromPointCloud(*cloud_full_dist_color,*cloud_full_dist_color, indices);
	ROS_INFO_STREAM("cloud_full_dist_color size:" << cloud_full_dist_color->size());
	if(cloudWrite){
		pcl::PLYWriter writer;
		writer.write ("cloud_full_dist_color.ply", *cloud_full_dist_color, false);
	}

	//----------------------------Voxellize and Slice-------------------------------
	ROS_INFO_STREAM("Size of cloud_full_dist_color: " << cloud_full_dist_color->size());
	ROS_INFO("Initializing cloud_full_xyz");
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_full_xyz (new pcl::PointCloud<pcl::PointXYZ>);	
	copyPointCloud(*cloud_full_dist_color, *cloud_full_xyz);
	if(cloudWrite){
		pcl::PLYWriter writer;
	...
	ne.setRadiusSearch (0.01); //TODO: perhaps change this to a number of neighbors
    ...
    */   
    ...
    for(int n=0; n<num_slices; n++){
	    ...
	    pass.setInputCloud (cloud_full_xyz_vox);
	    ...
	    for(int q=0; q<hotspots.size(); q++){
	    	if(hotspots[q][0]!=0){
				...
				if(sqrt(pow(centroid_x,2)+pow(centroid_y,2))<arm_extended && height_middle<(middle_start-0.2)){
