...
public:
	...
	std::vector<std::vector<float> > hotspotfilterFunction(pcl::PointCloud<pcl::PointNormal>::Ptr cloud);
...
void Stitch::callback(const boost::shared_ptr<const nav_msgs::Odometry>& odomPtr, const boost::shared_ptr<const sensor_msgs::PointCloud2>& cloudPtr, const boost::shared_ptr<const sensor_msgs::JointState>& jointPtr)
{
	...
	std::cout << "End of callback iteration" << std::endl;
}

std::vector<std::vector<float> > Stitch::hotspotfilterFunction(pcl::PointCloud<pcl::PointNormal>::Ptr cloud)
{
...
//-------------------Apply filtering based on normals and x-density-----------------------
	...
	std::cout << "got getMinMax3D" << std::endl;	
	...
	hotspots.resize(30, std::vector<float>(3, initial_value));
	...
	int noise_threshold = 1000;//40;
	int hotspot_threshold = 4000;
	int I;
	int J;
	int counter = 0;
	...
	for(int i=0; i<no_of_rows; i++)
	{
		for(int j=0; j<no_of_cols; j++) 
		{
			if(matrix[i][j]>hotspot_threshold){
				...
				outFile << i << " " << j << " " << 0 << "\n";  //this is to catch the elements of the vector that have not been filled (points to memory locs)				
			}
		}
	}
	outFile.close();
	...
	for(int i=0; i<no_of_rows; i++)
	{
		for(int j=0; j<no_of_cols; j++) 
		{
			...
			if ((matrix[i][j]>hotspot_threshold)) { //&& (matrix_normals[i][j])<0.5) {
    			...
    			while(queue.size()){
					for(int k=-1; k<2; k++){  //this goes in a 3x3 grid (?): -1, 0, 1
						for(int l=-1; l<2; l++){
							new_i = queue[0][0];
							new_j = queue[0][1];
...
void Stitch::slice(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_full, Eigen::Matrix4f GlobalTransform_odom){
    ...
    for(int n=0; n<num_slices; n++){
	    ...
	    try{
	    	hotspots = Stitch::hotspotfilterFunction(cloud_slice); 
	    }
