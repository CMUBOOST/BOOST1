...
void Stitch::filterStitch(boost::circular_buffer<sensor_msgs::PointCloud2> ringbuffer, boost::circular_buffer<nav_msgs::Odometry> odom_buffer)
{
	...
	for (int i=0; i<5; i++){
		...
		double dist_traveled = odom_buffer[i].pose.pose.position.x-odom_buffer[0].pose.pose.position.x;
		ROS_INFO_STREAM("DIST TRAVELED: " << dist_traveled << "\n");
		ROS_INFO_STREAM("STARTING POSE: " << odom_buffer[0].pose.pose.position.x << "\n");
		ROS_INFO_STREAM("CURRENT POSE: " << odom_buffer[i].pose.pose.position.x << "\n");
...
void Stitch::slice(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_full, Eigen::Matrix4f GlobalTransform_odom){
    ...
    for(int n=0; n<num_slices; n++){
	    ...
	    pass.setFilterLimits (.1-0.1*(n), .3-0.1*(n));
	    ...
	    std::cout << "after hotspots" << std::endl;   
		...
		//cout << "initialized spheres for display" << endl;
		...
		for(int i=0; i<hotspots.size(); i++){
			if(hotspots[i][0]!=0){
			    sphereCloud->points[globalSphereCount].x = 0.2-0.1*(n);
			    ...
			    sphere_locs_3d[n][i][0] = 0.2-0.1*(n);
	  			sphere_locs_3d[n][i][1] = hotspots[i][1];
	  			sphere_locs_3d[n][i][2] = hotspots[i][0];

				marker_sphere.header.frame_id = "/multisense/head";
			    marker_sphere.header.stamp = ros::Time::now();
			    marker_sphere.ns = ss9.str();
			    marker_sphere.id = i;
			    std::cout << "Sphere iterator: " << i << std::endl;
			    marker_sphere.type = shape;
			    marker_sphere.action = visualization_msgs::Marker::ADD;
				marker_sphere.pose.position.x = 0.2-0.1*(n);
	    		marker_sphere.pose.position.y = hotspots[i][1];
	    		marker_sphere.pose.position.z = hotspots[i][0];
    ...
    pass_new2.setFilterLimits (-1.8, -1.2);
    pass_new2.filter (*cloud_a_rgb);
	*/


	//viewer->setBackgroundColor (1, 1, 1); //.5, .5, 1
  	//pcl::visualization::PointCloudColorHandlerRGBField<pcl::PointXYZRGB> rgb(cloud_full);
  	//viewer->addPointCloud<pcl::PointXYZRGB> (cloud_full, rgb, "sample cloud");
	...
	cylinder_coeff.values.resize (7);    // We need 7 values
  	for(int i=0; i<sphere_locs.size(); i++){
  		...
  		if(std::find(to_remove.begin(), to_remove.end(), i) == to_remove.end()) {  //check if 'i' is one of the cylinders we want to remove
		    ...
		    marker.header.frame_id = "/multisense/head";
