...
bool burst_callback(std_srvs::Trigger::Request  &req, std_srvs::Trigger::Response &res)
{
  ...
  if (count==0) {
  	syncboard.write("b"); //this should probably be changed to only trigger the first time a callback is called
  ...
  //while(ros::ok()){  //This is dangerous. If a message is missed, the robot keeps moving.
  for(int i=0; i<5; i++){  //I think this value 150 is totally arbitrary. Can't remember...
    std::string result = syncboard.readline();
    output.linear.x = 0.5;		
    output.linear.y = 0.0;
    output.linear.z = 0.0;
