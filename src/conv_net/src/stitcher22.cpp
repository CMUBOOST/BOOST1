...
private:
	...
	const static float cam_z_offset = 0.05; //0.005; //0.015;	
...
//-----Run a loop that reduces the correspondence distance each time the TF drops below the threshold pairAlign_tform_eps
  	...
  	if(reg.getFitnessScore() > 0.004){
  		ROS_WARN_STREAM("Fitness score: " << reg.getFitnessScore());
  	}
...
void Stitch::callback(const boost::shared_ptr<const nav_msgs::Odometry>& odomPtr, const boost::shared_ptr<const sensor_msgs::PointCloud2>& cloudPtr, const boost::shared_ptr<const sensor_msgs::JointState>& jointPtr)
{
	...
	if (cb.size() == num_images){
		...
		ros::ServiceClient client0 = mNode.serviceClient<hebi::ArmConfigure>("arm_configure");
		hebi::ArmConfigure srv0;

		srv0.request.arm_config_req = "left";

		ROS_INFO("Populated the LEFT ARM_CONFIG request");
		if (client0.call(srv0))
		{
			ROS_INFO_STREAM("Response from Hebi Command: " << srv0.response.arm_config_resp);
		}
		else
		{
			ROS_ERROR("Failed to call service arm_configure upon initial request LEFT");
		}

	    ros::ServiceClient client = mNode.serviceClient<hebi::StageImmediate>("stage_immediate");
		hebi::StageImmediate srv;
		srv.request.stage_immediate_req = -.085; //this positions the arm at the very bottom of the stage
		//-24.1*height_middle;

		ROS_WARN("Populated the STAGE request");
		if (client.call(srv))
		{
			ROS_INFO_STREAM("Response from Hebi Command: " << srv.response.stage_immediate_resp);
		}
		else
		{
			ROS_ERROR("Failed to call service stage_immediate");
			//return 1;
		}
		ROS_WARN("ENDED the STAGE request");
		...
		if((double(stitching_time - begin) / CLOCKS_PER_SEC) < 2.8){
			usleep(2800000-(double(stitching_time - begin) / CLOCKS_PER_SEC)*1000000);
		}		
		...
		*/
        ...
        output.position.push_back(0);
        output.position.push_back(-3.57);
        zPub.publish(output);
        sleep(3);
        */
	...
	for(int i=0; i<no_of_rows; i++)
	{
		for(int j=0; j<no_of_cols; j++) 
		{
			...
			if ((matrix[i][j]>hotspot_threshold)) { //&& (matrix_normals[i][j])<0.5) {  //i.e. find a random hotspot
    			...
    			ROS_INFO_STREAM("loc_x: " << loc_x << ", loc_z: " << loc_z);
...
void Stitch::slice(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_full, Eigen::Matrix4f GlobalTransform_odom){
	...
	//-----------------------Filter out points that are more red than green---------------------------------
    ...
    //copyPointCloud(*cloud_vox_dist_color, *cloud_vox_dist_color_new);
	...
	normalEstimation.setInputCloud (cloud_vox_dist_color_new);
	...
	/*
	// Set up the principal curvatures computation
	...
	principalCurvaturesEstimation.setInputCloud (cloud_vox_dist_color_new);
	...
	principalCurvaturesEstimation.setRadiusSearch(0.015);
	...
	for(int m=0; m<cloud_vox_dist_color_new->size(); m++){
		cloud_vox_dist_color_new->points[m].r = int((125*principalCurvatures->points[m].principal_curvature[0])+125);
		cloud_vox_dist_color_new->points[m].g = int((125*principalCurvatures->points[m].principal_curvature[1])+125);
		cloud_vox_dist_color_new->points[m].b = int((125*principalCurvatures->points[m].principal_curvature[2])+125);				
	}
	cloudWriter(cloud_vox_dist_color_new, "cloud_vox_dist_color_new_colored.ply", cloudWrite);
	*/
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_vox_dist_color_new_normals (new pcl::PointCloud<pcl::PointXYZRGB>);
	std::vector<float> colorval;
	colorval.resize(3);
	for(int m=0; m<cloud_vox_dist_color_new->size(); m++){
		/*
		if ((382*cloudWithNormals->points[m].normal[1]+382)>255){
			cloud_vox_dist_color_new->points[m].r = 255;
			if((382*cloudWithNormals->points[m].normal[1]+382-255)>255){
				cloud_vox_dist_color_new->points[m].g = 255;
				cloud_vox_dist_color_new->points[m].b = 127*cloudWithNormals->points[m].normal[1]+127;
			}
			else{
				cloud_vox_dist_color_new->points[m].g = 127*cloudWithNormals->points[m].normal[1]+127;
				cloud_vox_dist_color_new->points[m].b = 0;				
			}

		}
		else{
			cloud_vox_dist_color_new->points[m].r = 127*cloudWithNormals->points[m].normal[1]+127;
			cloud_vox_dist_color_new->points[m].g = 0;
			cloud_vox_dist_color_new->points[m].b = 0;
		}
		*/

		if(cloudWithNormals->points[m].normal[1] < 0.2 && cloudWithNormals->points[m].normal[1] > -0.2){
			cloud_vox_dist_color_new_normals->push_back(cloud_vox_dist_color_new->points[m]);
		}		
	}
	//copyPointCloud(*cloud_vox_dist_color_new, *cloud_vox_dist_color_new_normals);

	cloudWriter(cloud_vox_dist_color_new, "cloud_vox_dist_color_new_normals_colorized.ply", cloudWrite);
	cloudWriter(cloud_vox_dist_color_new_normals, "cloud_vox_dist_color_new_normals.ply", cloudWrite);
    ...
    for(int n=0; n<num_slices; n++){
	    ...
	    pcl::PassThrough<pcl::PointXYZRGB> pass;
	    pass.setInputCloud (cloud_vox_dist_color_new_normals);
	    ...
	    ROS_DEBUG_STREAM("above cloud slice filter");
	    ...
	    try{
	    	...
	    	ROS_WARN("Hotspotfilterfxn EXCEPTION");
	    }
	    ...
	    for(int q=0; q<hotspots.size(); q++){
	    	if(hotspots[q][0]!=0){		
				...
				kdtree.setInputCloud (cloud_vox_dist_color_new_normals);
				...
				if ( kdtree.radiusSearch (searchPoint, radius, pointIdxRadiusSearch, pointRadiusSquaredDistance) > 0 ){
					ROS_DEBUG_STREAM("Number of neighbors: " << pointIdxRadiusSearch.size ());
					for (size_t i = 0; i < pointIdxRadiusSearch.size (); ++i){
						green+=cloud_vox_dist_color_new_normals->points[ pointIdxRadiusSearch[i] ].g; //green value will be comapred to a threshold
						x+=cloud_vox_dist_color_new_normals->points[ pointIdxRadiusSearch[i] ].x;  //xyz location will be averaged for better centroid placement
						y+=cloud_vox_dist_color_new_normals->points[ pointIdxRadiusSearch[i] ].y;
						z+=cloud_vox_dist_color_new_normals->points[ pointIdxRadiusSearch[i] ].z;																		
					}
					...
					ROS_DEBUG_STREAM("Green total: " << green << ", Green avg: " << greenavg);
				}
				...
				if(greenavg > greenthresh && pointIdxRadiusSearch.size() > 20){
				    ...
				    if(sqrt(pow(centroid_x,2)+pow(centroid_y,2))<arm_extended && height_middle<(middle_start-0.1)){
				    	...
				    	if(flag==0){
						    ...
						    //std::cout << "Radius seach: " << pointIdxRadiusSearch.size() << std::endl;
							...
							{
								ROS_ERROR("Failed to call service arm_configure LEFT before moving stage");
							}
						    
						    ros::ServiceClient client = mNode.serviceClient<hebi::Stage>("stage");
							hebi::Stage srv;
							srv.request.stage_req = -height_middle-.035; //this offset is because the vertical position of the arm in RVIZ appears a little off
							ROS_WARN_STREAM("Stage Request: " << -height_middle-.035);

							ROS_INFO("Populated the request");
							if (client.call(srv))
							{
								ROS_INFO_STREAM("Response from Hebi Command: " << srv.response.stage_resp);
							}
							else
							{
								ROS_ERROR("Failed to call service stage before servoing to marker");
								//return 1;
							}

						  	ROS_WARN("Calling ArmInitStitch client");
							ros::ServiceClient client2 = mNode.serviceClient<arm_executive::ArmInitStitch>("arm_init");
							arm_executive::ArmInitStitch srv2;
						  	srv2.request.arm_init_req = "left";
						  	srv2.request.arm_init_req_x = centroid_x;
						  	srv2.request.arm_init_req_y = centroid_y;

						  	if (client2.call(srv2))
						  	{
						    	ROS_INFO_STREAM("Response: " << srv2.response.arm_init_resp);
						  	}
						  	else
						  	{
						    	ROS_ERROR("Failed to call service arm_init to servo to marker");
						  	} 
					...
					ROS_WARN_STREAM("Out of reach!! Reach: " << sqrt(pow(centroid_x,2)+pow(centroid_y,2)));
				}
				else if(pointIdxRadiusSearch.size() <= 20){
					ROS_WARN_STREAM("Not enough neighbors: " << pointIdxRadiusSearch.size());
				}				
				else
					ROS_WARN_STREAM("Not green enough: " << greenavg);
			}
		} 
    ...
    ros::ServiceClient client = mNode.serviceClient<hebi::Stage>("stage");
	hebi::Stage srv;
	srv.request.stage_req = 0; //this offset is because the vertical position of the arm in RVIZ appears a little off
	ROS_INFO("Populated the request");
	if (client.call(srv))
	{
		ROS_INFO_STREAM("Response from Hebi Command to reset stage: " << srv.response.stage_resp);
	}
	else
	{
		ROS_ERROR("Failed to call service stage as final move after slicing");
	}


	client = mNode.serviceClient<hebi::ArmConfigure>("arm_configure");
	hebi::ArmConfigure srv2;

	srv2.request.arm_config_req = "stow";

	ROS_INFO("Populated the STOW ARM_CONFIG request");
	if (client.call(srv2))
	{
		ROS_INFO_STREAM("Response from Hebi Command: " << srv2.response.arm_config_resp);
	}
	else
	{
		ROS_ERROR("Failed to call service arm_configure to stow arm");
	}
...
int main(int argc, char** argv)
{
	...
	std::cout << "Ready to stitch" << std::endl;
  	...
  	message_filters::Subscriber<sensor_msgs::PointCloud2> cloudSub(nh, "/multisense/image_points2_color", 50);
  	message_filters::Subscriber<sensor_msgs::JointState> jointSub(nh, "/joint_states", 50);

  	typedef sync_policies::ApproximateTime<nav_msgs::Odometry, sensor_msgs::PointCloud2, sensor_msgs::JointState> MySyncPolicy;

  	Synchronizer<MySyncPolicy> sync(MySyncPolicy(50), odomSub, cloudSub, jointSub);
