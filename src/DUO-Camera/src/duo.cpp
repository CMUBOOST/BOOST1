...
bool DUOStereoDriver::initializeDUO()
{
	 ...
	 */
	_priv_nh.getParam("frame_id", _camera_frame);
	...
	_priv_nh.getParam("FPS", framesPerSecond);
	...
	int 	resWidth;
	int 	resHeight;
	_priv_nh.getParam("resolution_width", 	resWidth);
	_priv_nh.getParam("resolution_height", resHeight);
    ...
    _priv_nh.param<bool>("use_DUO_calibration",  _useDUOCalibration,  false);
	...
	else
	{
		...
		_priv_nh.getParam("camera_info_left", camera_info_url_left);
		_priv_nh.getParam("camera_info_right", camera_info_url_right);
	...
	if(EnumerateResolutions(&_duoResolutionInfo, 1, resWidth, resHeight, binning, framesPerSecond))
	{
		...
		if(OpenDUO(&_duoInstance))
		{
			...
			//_priv_nh.param("LED"			, _duoLEDLevel	, 50.0);

			_priv_nh.getParam("exposure"		, _duoExposure);
			_priv_nh.getParam("gain"		, _duoGain);
			_priv_nh.getParam("LED"			, _duoLEDLevel);
