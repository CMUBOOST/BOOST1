...
int RearHoming(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode, std_msgs::Int32* stalk_dia_msg)
{
    ...
    int CurrentThresh = 500; //Current threshold in mA
...
public:
    Gripper();
    bool grip(gripper::GripperActuate::Request  &req,
          gripper::GripperActuate::Response &res);

private:
    ros::NodeHandle nh;
    ros::Publisher start_pub;
    std_msgs::Int16 start_msg;
};

Gripper::Gripper(){
    this->start_pub = nh.advertise<std_msgs::Int16> ("start", 10);
}


bool Gripper::grip(gripper::GripperActuate::Request  &req,
          gripper::GripperActuate::Response &res)
{
    ...
    res.gripper_actuate_resp = req.gripper_actuate_req;  //this is a placeholder for now

    ROS_INFO_STREAM("Gripper actuation request: " << req.gripper_actuate_req);

    //ros::Publisher start_pub=nh.advertise<std_msgs::Int16>("start", 1);
    ...
    ROS_INFO_STREAM("start msg: " << start_msg.data);
    ...
    ROS_INFO_STREAM("sending back response: " << res.gripper_actuate_resp);
...
int main(int argc, char **argv)
{
  ...
  ros::ServiceServer service = n.advertiseService("gripper_actuate", &Gripper::grip, pGripper);
