...
int   GripperForward(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode);
int   CloseGripper(unsigned int* p_pErrorCode);
...
{
    int lResult = MMC_SUCCESS;
    int long closePosition = -22000*4;
    int vel = 10000;
    int accel = 10000;
    int decel = 10000;
...
int CloseGripper(unsigned int* p_pErrorCode)
{
    ...
    lResult = GripperForward(g_pKeyHandle, g_usNodeId, lErrorCode);

    ROS_INFO_STREAM("lResult is:" << lResult);

    if(lResult != MMC_SUCCESS)
    {
        LogError("GripperForward", lResult, lErrorCode);
    }
          ...
          gripper::GripperClose::Response &res)
{
    ...
    if((lResult = CloseGripper(&ulErrorCode))!=MMC_SUCCESS)
	{
        LogError("CloseGripper", lResult, ulErrorCode);
		return lResult;
	}
